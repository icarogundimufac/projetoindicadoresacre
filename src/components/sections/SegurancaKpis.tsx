import { TrendingUp, TrendingDown, ShieldAlert, AlertTriangle } from 'lucide-react'
import { formatNumber } from '@/lib/utils/format'
import type { IndicatorSection } from '@/types/indicators'

interface SegurancaKpisProps {
  data: IndicatorSection
}

export function SegurancaKpis({ data }: SegurancaKpisProps) {
  ...
  // ── Card esquerdo: Taxa de Homicídios ──
  const homicidios = data.groups
    .find((g) => g.id === 'crimes_violentos')
    ?.indicators.find((ind) => ind.id === 'taxa_homicidios')

  // ── Card direito: Taxa de Roubos ──
  const roubos = data.groups
    .find((g) => g.id === 'crimes_patrimonio')
    ?.indicators.find((ind) => ind.id === 'taxa_roubos')
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
      ...
      <div className="rounded-xl border border-areia-200 bg-white px-4 py-3 shadow-sm">
        <div className="flex items-center gap-1.5 mb-1">
          <AlertTriangle size={12} className="text-ouro-500" />
          <p className="text-[10px] font-bold uppercase tracking-wider text-areia-400 font-jakarta">
            Roubos
          </p>
        </div>
        {roubos && roubos.latestValue !== undefined ? (
          <>
            <p className="text-xl font-bold font-fraunces text-verde-900 tabular-nums leading-tight">
              {formatNumber(roubos.latestValue)}
            </p>
            <p className="text-[10px] text-areia-400 font-jakarta mt-0.5">
              {roubos.unit} — {roubos.timeSeries.at(-1)?.year}
            </p>
          </>
        ) : (
          <p className="text-sm text-areia-400 font-jakarta">—</p>
